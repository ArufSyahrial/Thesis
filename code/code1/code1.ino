...
#define I2S_SD_L    GPIO_NUM_40  // Mikrofon Kiri (SD)
#define I2S_SD_R    GPIO_NUM_39  // Mikrofon Kanan (SD) - akan di-wire ke SD
...
void setup() {
  ...
  Serial.println("\n\n");
  Serial.println("╔════════════════════════════════════════════════════╗");
  Serial.println("║  🚦 ESP32-S3 SMART TRAFFIC SYSTEM v2.0 🚦        ║");
  Serial.println("║     Dual Microphone Ambulance Detection          ║");
  Serial.println("║     Fixed & Optimized Version                     ║");
  Serial.println("╚════════════════════════════════════════════════════╝");
  Serial.println();
  ...
  static tflite::MicroMutableOpResolver<20> resolver;  // tingkatkan kapasitas

  // Daftar semua operator yang mungkin digunakan model
  resolver.AddFullyConnected();
  resolver.AddSoftmax();
  resolver.AddReshape();
  resolver.AddQuantize();
  resolver.AddDequantize();
  resolver.AddExpandDims();   // <<=== tambahkan ini
  resolver.AddConv2D();       // opsional (kalau model audio pakai konvolusi)
  resolver.AddMean();         // opsional (sering muncul di model audio)
  resolver.AddMul();          // opsional (kadang ada di normalisasi)
  resolver.AddAdd();          // opsional (kadang dipakai layer residual)

  // Selesai setup resolver
